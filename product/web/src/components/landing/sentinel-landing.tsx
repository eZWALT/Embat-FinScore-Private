...
const NAV_LINKS = [{ href: "#team", label: "Team" }] as const;
          ...
          <ul className="flex items-center gap-6">
            {NAV_LINKS.map((link) => (
              <li key={link.href} className="hidden sm:block">
                <Link href={link.href} className="text-sm text-neutral-400 transition-colors hover:text-black">
                  {link.label}
                </Link>
              </li>
            ))}
            <li>
              <Link
                href="/empresa"
                className="inline-flex items-center bg-black px-3.5 py-2 text-sm text-white"
              >
                Demo
              </Link>
            </li>
