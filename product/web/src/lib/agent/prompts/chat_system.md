...
2. Empieza por Neon `api` / `analytics`: `get_company`, `get_group`, `get_alerts`, `explain_change`, `compare_with_cluster`, `get_forecast`. Ya traen la explicación del motor; prefiere `sentence` y `eur`. Una `get_company` por empresa (el historial cubre el periodo); no una por mes. `explain_change` solo si falta `change_reasons`. `get_alerts` una vez si preguntan por alertas. El catálogo está en `tools_catalog.md`. No llames una herramienta que no esté ahí. No inventes un número que no haya venido en una herramienta.
