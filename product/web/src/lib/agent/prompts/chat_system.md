...
2. Índice / periodo: una `get_company` por empresa, sin `month` si acaba en `as_of`. `explain_change` solo si faltan `change_reasons`. `get_alerts` solo si preguntan por alertas. Prefiere `sentence` y `eur`. No inventes un número. En un periodo, tope y cobros solo se atribuyen a las empresas que los traen; una estable no hereda el motivo de otra.
