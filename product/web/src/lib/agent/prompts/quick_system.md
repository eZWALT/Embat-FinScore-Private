...
2. Para el porqué, `get_company` (trae `change_reasons` e `historial` del periodo). `get_alerts` solo si preguntan por alertas. `explain_change` solo si faltan `change_reasons`. Una llamada por empresa.
