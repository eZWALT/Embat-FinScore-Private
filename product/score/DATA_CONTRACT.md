# Health Score data contract (schema 1.3.0)
...
| `guard` + `guard_ceiling` | `dark`: no bank booking for 60 days, cap 30. `fading`: inflows collapsed vs own history, cap 50. Since 1.3.0 the cap is a **ceiling that glides**: while a guard is on, the score cannot exceed `guard_ceiling`, which starts from the company's previous score and comes down at most `spec.guard.max_drop_per_month` (10) points a month toward the cap, and lifts at once when the guard ends. So switching a guard on (often a gap in the feed rather than a real stop) cannot turn into a 20-40 point step, and a one-month gap costs at most 10 points; a persistent silence still reaches the cap. `score_pre_cap` is what the score would be without the guard, `guard_adjustment` = `score - score_pre_cap`. **Show the guard when it is on:** a raw 90 on a silent company is the trap this exists for, and a bank feed that drops looks the same as a company that stops. The `going_dark` alert still fires in the first month. |
