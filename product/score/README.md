...
**Going dark scores low, never high** (`spec.py` guard). Outflow ratios improve and invoice delays vanish when activity dies: on the real data the raw score of a company silent for 200–500 days is ~90. So: no booking of any kind in the 60 days before month-end → **dark**, cap 30, confidence low, trajectory `deteriorating`; last-3-month inflow under 25% of the company's own earlier 6-month mean → **fading**, cap 50. **The cap glides** (schema 1.3.0, `GUARD_STEP`): while a guard is on, the ceiling starts from the previous score and comes down at most 10 points a month toward the cap and lifts at once when the guard ends, so a gap in the data cannot cause a 20-40 point step (month-on-month falls of 20+ points: 467 → 48). Tested in `guard_test.py` on 60 companies from the top 40% by score whose data stops after 2026-02 (bank only, and bank + invoices): every dark row is under its ceiling, which falls at most 10 a month toward 30 (the median company is at the cap in month 6), none above the pre-cutoff score, from month 3 every company is flagged; with the guard off the median post-cutoff score stays 71–87. The gap: the first silent month is not yet flagged (7% of rows in months 1–2 score above the pre-cutoff score, largest rise +8.9 points). A bank connection that drops looks the same as a company that stops; the score cannot tell them apart.
