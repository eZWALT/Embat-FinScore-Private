...
Fixed weights and percentiles are deliberate: they are documented, explainable, and cannot overfit any outcome. Sensitivity check: 60 random reweightings (±30%) keep Spearman ≥ 0.977 with the base score and move AUROC by at most 0.013.

### 3.3 Guard, confidence, trajectory

- **Going-dark guard.** No bank booking of any kind for 60 days: score capped at **30** (`guard = dark`). Last-3-month inflows under 25% of the company's own earlier 6-month mean: capped at **50** (`fading`). Both are shown with the score before the cap. Why: several ratios (runway, debt service over inflows) *improve* when activity dies, so without a cap a silent company could score high.
...
|---|---|---|
| Score falls (≥ 8 points, 3 of 4 months) and going dark | Followed by an accepted outcome within 6 months 29% of the time, vs 31% for an alert on a random month; lift 0.72-1.24 per outcome; median lead time 2 months where followed | Descriptive: says what moved, why and by how much. Does not predict |
...
| Volume | 0.55 risk and 0.20 improvement alerts per company-year | 47% of one-month falls of 8+ points became an alert within 3 months; the rest reverted |
| Group funnel | 2.4% of train group-months outside 3σ (heavy tails), 0.3% persistently | Groups under 3 members get no limits |
