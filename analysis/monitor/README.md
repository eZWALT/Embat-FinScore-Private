...
- **Forecast: the score is not a trending series, so the gain is in mean reversion and in the shape of the fan.** Monthly changes are negatively autocorrelated (lag 1: −0.14) and the score is pulled toward the company's own average and the portfolio level. Damped trend (Holt) loses to the last value by 0.3–5% MAE at 1–3 months and ties at 4–6 (at most +0.4%), so there is no tendency to extrapolate. Per-company ARIMA (220 sampled companies) is 1–27% worse at 1–3 months and, at 4–6, from 6% worse to 3.5% better: only the mean-reverting AR(1) with a constant gains, which is the effect the pooled model uses, estimated on the whole portfolio instead of one short series. The shipped **reversion_quantile** fan (linear quantile regression per horizon, seven features, pooled over train companies) beats the naive fan on pinball loss at every horizon with intervals above zero: **+3.3% (1 month), +7.3, +11.5, +13.5, +14.9, +15.9% (6 months)**, with 50% and 80% intervals that cover 50% and 80% out of fold, also by score level, history length and volatility (worst cell 77% for the 80% interval). A stricter check that also holds out the later months (fit on origins up to 2025-09, test on held-out companies afterwards) keeps the gain but smaller, **+1.5% at 1 month to +11.6% at 6**, and the 80% interval then covers 74–78%: read the first numbers as the optimistic end. The median alone gains little (MAE −0.1% at 1 month, +1.0% at 3, +4.7% at 6): most of the improvement is that the fan is skewed and moves with the company's deviation from its own average. **No seasonality**: lag-12 autocorrelation of monthly changes −0.014 (95% CI −0.05 to +0.03) and the calendar-month means do not repeat in the second year (r = −0.21); 24 months rule out a strong yearly pattern, not a weak one. **Moves are heavy-tailed**: after a fall of 8 or more points in 3 months (median −15) the median next 3 months is +0.3 and only 28% of cases recover at least half, while the mean is +3.9 because of a minority of large reversals; after a rise the typical case gives back 1.8 points. It is still a persistence fan, not a prediction of outcomes.
