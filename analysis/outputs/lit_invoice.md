...
| 7 | Irvine, Park, Yıldızhan, “Customer-Base Concentration, Profitability, and the Relationship Life Cycle” | 2016 | *The Accounting Review* 91(3) | [doi](https://doi.org/10.2308/accr-51246) · [WP PDF](https://mpra.ub.uni-muenchen.de/53886/1/MPRA_paper_53886.pdf) | Concentration is **not monotone**: unprofitable CC mean **0.142** vs profitable **0.090**. WP Table 9: **Rank(CC)** raises IPO failure over **5 and 7 years** (weaker at 7). Table 10: Rank(CC)+, AGE×Rank(CC)−. Loss of a major customer is the crash. | Q4, Q5 |
...
### Wave C — Credit-note *dispute vs distress* on the same invoices (footnote only)

- **Owner files:** `analysis/evaluate/cn_dispute_qa.py` + md.
- **Forbidden X:** grow TURNOVER; invent `y_credit_note`; CN as Y3 X; merge J.
- **Acceptance:** split CN into **note vs refund** and/or CN **on the top-1 counterparty** vs rest. Leftover after issued_lag1 **and** the who-uses-notes dummy. If the month-shock leftover stays **< 0.55**, CLOSE — the 0.597 is the trait.
- **Why literature asks:** Boissay’s defaults are **mostly disputes** (16.2% vs 2.1% illiquidity; dispute tickets smaller). Second-pass search still found **no PD paper** on credit-note ratios — only collections models that treat a dispute *flag* as a lateness feature. This is the only honest next cut.
- **Why not a redo:** `credit_note_qa` already showed demean dies. This asks *which invoices* get notes.
...
- **Forbidden X:** `d_supp_hhi` as engine X; merge Y4; `d_tx` as leave-one-group law; Family E on the Y5 GBM; grow TURNOVER.
- **Acceptance:** on the **65% neither-cash-nor-HHI** AP positives, leftover of **net TC** (`e_ap_open − e_ar_open`, or issued vs AP stock) **× a quiet/activity flag** (days or `a_in3` dip) **≥ 0.58** after the cash+HHI dummy, beat-size, not fold-3 `d_tx`. If nothing clears, the 65% stays the sentence.
- **Why literature asks:** Bureau–Duquerroy–Vinas: 1 SD net TC raises **payment-default** PD **+10% only in the shock months**; quiet months **zero**. That is our Y5 object (we start paying suppliers late) and our leftover (cash already taken). García-Appendini / Boissay explain why the HHI tail is protective. Pérez-Salazar said the opposite on synthetic data.
- **Why not a redo of Y5:** not the supplier-HHI tail, not the 0.611 `d_tx` quote, trees stay PARK, E stays off the Y5 card.
